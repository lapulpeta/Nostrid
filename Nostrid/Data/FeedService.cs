...
public class FeedService
{
	...
	private BlockingCollection<Event>[] eventListQueues;
	private CancellationTokenSource queuesCancellationTokenSource;

	public int MaxQueues => Environment.ProcessorCount;
	...
	{
		queuesCancellationTokenSource = new CancellationTokenSource();
		eventListQueues = new BlockingCollection<Event>[MaxQueues];
	...
	{
		queuesCancellationTokenSource.Cancel();
	}

	private void HandleEvents(BlockingCollection<Event> queue, CancellationToken cancellationToken)
	{
		while (!cancellationToken.IsCancellationRequested)
		{
			try
			{
				var eventToProcess = queue.Take(cancellationToken);
	...
	private void EnqueueEvent(IEnumerable<Event> events)
	{
		foreach (var ev in events)
		{
			var queueIndex = ev.Id.Last() % MaxQueues;
			eventListQueues[queueIndex].Add(ev);
		}
	}
